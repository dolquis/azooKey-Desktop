...
- レビュー対象の全差分と対象の未追跡ファイルは内容まで確認する。大量の diff は Context-Mode 等でファイルごとに解析し、指摘・根拠位置・必要な原文だけを返す。要約だけで確認したことにしない。
- 確認したソースと差分は再利用する。stage／commit 後は対象ファイル・包含範囲・内容の一致を確認し、一致する既読差分は再掲しない。編集、base の更新、鮮度警告、未解決事項がある範囲は再確認する。
- connector は必要フィールドへ絞り、予想外の構造はキーから調べる。ログ全体から終了コード、失敗 ID、重要 warning、根拠位置を抽出し、末尾の失敗も確認する。通常の返却目安は 4〜8 KiB とし、証拠は段階的に追加する。
- ローカル分担の共通規則は agent-ops の `linear-conventions.md` §2.1（製品 repo では `docs/linear-conventions.md`）を正典とする。利用可能なら独立した観点のレビューに適用し、不在時は親が同じ確認範囲を担当する。担当には必要な根拠と検証を指定し、返された指摘を親が実ファイルで検証する。
