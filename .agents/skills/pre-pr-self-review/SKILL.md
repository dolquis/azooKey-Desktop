...
- 独立した観点のレビューで品質改善や時間短縮が見込める場合はローカルサブエージェントを使う。対象・非対象、書込み境界、必要な根拠と検証、ユーザーの PR 本数・順序・完了条件を渡す。親が指摘を検証・統合し、同一ファイルや共有 Serena の対象変更を競合させない。小修正のための分割や全履歴の無条件な複製は避ける。
