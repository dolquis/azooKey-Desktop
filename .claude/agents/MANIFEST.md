...
「権限」列は checker が両側で突き合わせる契約である。`read-only` は Claude 側に
...
|---|---|---|---|---|---|
| `diff-auditor` | shared | `dolquis/agent-ops` | 未記録（repo 側 `b2d2cce`、`39af1e9` で更新） | read-only（origin の定義に従う） | なし（origin で追加予定。取り込みは Linear DEV-1275） |
