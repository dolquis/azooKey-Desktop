...
- 設定UI（`settings-app/`）の入力制約、表示、初期値、保存値。`settings/` には schema と sample だけがあり、UI 実装は無い
- 対応する `docs/*-spec.md`
...
- 設定アプリの保存と起動引数: `azookey_settings_persistence_tests`、`azookey_settings_launch_arguments_tests`（Windows 構成のみ登録）
