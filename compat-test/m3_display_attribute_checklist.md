...
C-001〜C-012 は `GUID_PROP_ATTRIBUTE` に載せた属性の描画を判定しないため、
自動化済みのアプリでも下線と配色は手動でしか観測できない。
どの項目を自動側に委ねられるかは §3.1 に定める。
...
### 3.1 自動化済みアプリで省略してよい項目

Notepad / Edge / VS Code は M50 ハーネスの C-001〜C-012（`compat-test/targets/*.json`）が
入力動線を自動で回す。この 3 アプリでは、自動側が同じことを判定する項目だけを省略できる。

| 本チェックリストの項目 | 自動側の対応ケース | 3 アプリでの扱い |
|---|---|---|
| D-01 / D-06 | C-001（`nihongo` → Space → Enter の確定） | 省略可（`N/A`） |
| D-04 | C-002（Backspace で preedit が戻る） | 省略可（`N/A`） |
| D-05 | C-003（ESC で composition 破棄） | 省略可（`N/A`） |
| D-02 / D-03 | なし | 実施する |
| D-07 | C-009 は crash しないことだけを見る | 実施する |
| D-08 / D-09 | C-006 は候補ウィンドウの位置だけを見る | 実施する |
| D-10 | なし | 実施する |

**3 アプリを丸ごと手動対象から外さない。** 自動側は下線と配色を一切判定しないため、
外すと「下線が出ない」「配色が崩れる」回帰が自動結果を根拠に見逃される。
§6 が既知差異として挙げているとおり、描画差が出やすいのはむしろ Chromium 系の Edge と
Monaco の VS Code である。

省略した項目は §7 の記録テンプレートで `N/A` とし、根拠として実行した compat run
（DEV-716 / DEV-684 で配線した optional compat CI ステージ）を併記する。
自動側の結果を確認せずに `N/A` としない。
...
この 3 本が自動化済みであることは省略の理由にならない。C-006 が見るのは候補ウィンドウの
位置であり、未確定文字列に対する下線の追従は判定しないためである（§3.1）。
