...
残りは時間の許す範囲で確認し、未実施は DEV-365 に「未実施」と明記する
（黙って落とすと「確認済み」と読まれるため）。
...
D-08 / D-09 は必須アプリのうち Notepad / Edge / VS Code で確認すれば足りる
（DPI 追従はアプリ固有ではなく TSF とアプリのレイアウト側の問題として現れる）。

## 5. 合格条件

- 必須アプリすべてで D-01 / D-04 / D-05 / D-06 / D-07 が期待どおりである。
- D-03 で文字が読めなくなる（前景色と背景色が同化する、選択範囲と区別できない）
  事象が 1 件もない。
- 確定後に下線が残る、未確定文字列が二重に表示される、composition がアプリを
  またいで持ち越される事象が 1 件もない。
- D-02 は Chromium 系（Edge / Chrome / Electron）と conhost 系で不一致が出うる
  （§6 の既知差異）。不一致は即 fail とせず、対象アプリと症状を DEV-365 に
  列挙し、follow-up Issue を起票してリンクする。
- D-10 の差分は合否に含めない。所見として記録する。
