...
BracketPairingAction EvaluateBracketInput(char32_t codepoint, bool alnum_mode,
  ...
  if (const auto pair = LookupBracketPair(codepoint)) {
  ...
  if (LookupClosingBracket(codepoint)) {
...
BracketPairingAction EvaluateBracketBackspace(bool alnum_mode, const EditContextHint& hint,
  ...
  const auto pair = LookupBracketPair(*hint.char_before);
  if (!pair || hint.char_after != pair->close) return {};
