...
Candidate MakeCandidate(const std::string& reading,
                        std::string surface,
...
std::vector<Candidate> ExpandKatakanaCandidates(const std::string& reading) {
  ...
  candidates.push_back(
      MakeCandidate(reading, std::move(fullwidth), "全角カタカナ", "fullwidth"));
  if (halfwidth_supported) {
    candidates.push_back(
        MakeCandidate(reading, std::move(halfwidth), "半角カタカナ", "halfwidth"));
  }
  return candidates;
}
