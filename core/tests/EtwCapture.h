...
inline EtwCaptureResult CaptureEtw(const std::function<void()>& emit) {
  ...
  properties->LogFileNameOffset =
      static_cast<ULONG>(sizeof(EVENT_TRACE_PROPERTIES) + (name.size() + 1) * sizeof(wchar_t));
  ...
  logfile.EventRecordCallback = [](EVENT_RECORD* record) {
    constexpr GUID expected{
        0x89553f1c, 0x7d18, 0x4a6d, {0xa7, 0x20, 0x53, 0x49, 0x21, 0xc8, 0xb7, 0x30}};
    if (!IsEqualGUID(record->EventHeader.ProviderId, expected) ||
        record->EventHeader.ProcessId != GetCurrentProcessId())
      return;
    const auto* bytes = static_cast<const unsigned char*>(record->UserData);
    static_cast<std::vector<EtwEvent>*>(record->UserContext)
        ->push_back({record->EventHeader.EventDescriptor.Id,
