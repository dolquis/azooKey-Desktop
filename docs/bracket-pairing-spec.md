...
composition トリガの確定では、`EndComposition` の前に composition range の現在のテキストを
読み切り、確定後に同じ 3 経路で末尾から 1 code unit 手前へカーソルを置く。読取には上限
（64 code unit）を設ける。読めない場合、読み切れない場合、または上限を超える場合は、
末尾基準の経路へ退避する。取消時はテキストを消した後の末尾へカーソルを置く。
