...
`Handshaking` から `Ready` へ遷移する前に旧世代向けの pending / in-flight 要求と
候補表示待機状態を破棄する。要求送信時の世代 ID と応答反映時の世代 ID も照合し、
一致しない応答は UI へ反映しない。世代の初回観測と変化は構造化ログへ記録する。
...
- Host 再起動をまたぐ再接続で `host_generation_id` の変化を検知・記録し、
  旧世代向けの pending / in-flight 要求と遅延応答を破棄する
