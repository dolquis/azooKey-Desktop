...
| ⚠️ | 要点 |
|---|---|
| **基本セッションで検証** | Hyper-V の拡張セッション（VMConnect = RDP ベース）では IME 入力がリダイレクトされ、VM 内 TIP の挙動が正しく見えないことがある。**ファイルコピーは拡張セッション、IME 検証は基本セッション**に切り替えて行う。唯一の例外は候補ウィンドウの位置だけを見る C-005 で、条件は下記「拡張セッションの限定例外（C-005）」が定める。 |
...
`compat-test` の C-005（マルチディスプレイ端で候補ウィンドウが画面内にクランプされること。ケース定義の正典は `docs/dev-infrastructure-spec.md` §13.3）に限り、拡張セッションでの実施を認める。

Hyper-V の基本セッションは単一ディスプレイしか持てず、複数モニター構成は拡張セッションの機能である。一方 C-005 の判定材料は候補ウィンドウの表示位置であり、描画側にある。入力経路のリダイレクトが判定を歪める余地は、打鍵結果そのものを判定する他のケースより小さい。

例外はこの 1 ケースに閉じる。同じ拡張セッションで A 系・B 系の打鍵チェックや C-005 以外の compat ケースを実施しない。

**実施前に前提を確認する。** 拡張セッションでは入力がリダイレクトされ、VM 内 TIP の preedit が現れない、または正しく観測できないことがある。その状態では候補ウィンドウも観測できず、C-005 を判定できない。

1. 拡張セッションでメモ帳を開き、azooKey を選択して `nihongo` と打鍵する。
2. 下線付きの preedit が現れることを確認する。
3. preedit が現れない場合は C-005 を `failing-skip` として記録し、実施しない。候補ウィンドウが出ないことを Fail として記録しない。

preedit が出ない状態で得た観測は候補ウィンドウの位置を示さないため、C-005 の合否を表さない。
