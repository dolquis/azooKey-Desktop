<!-- lint:allow-file line-ref,status -->
<!-- 本ファイルは作成日時点の調査スナップショットであり、更新しない前提の記録である。 -->
...
> - 取り込みの追跡（Linear, team Dev）→ DEV-397（Tracking）/ 子課題 DEV-398〜413（M62 リライター・モデル管理・設定・CLI 等）
> - **本レポート内の `優先度` / `タイミング` / Phase は調査時点（作成日）の推奨であり、本ファイルは更新しない。** 現行の優先度・状態は Linear（DEV-397 と子課題 DEV-398〜413）を唯一の正典とし、乖離した場合は Linear を正とする（AGENTS.md の「進捗・状態・優先度は Linear に一本化」方針に従う。本表は分析スナップショットであってトラッカーではない）。
>
