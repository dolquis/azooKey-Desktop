...
承認されたローカル作業内のサブエージェント分担は、この Cloud 起動とは区別する。品質改善または時間短縮が見込める独立した調査・実装・レビューには、利用可能な枠内で積極的に分担する。目的、対象・非対象、書込み境界、成果形式、検証、PR 本数・Issue 順序・完了条件を渡す。親が統合と最終検証を担い、共有ファイル・build directory・Serena の対象変更の競合を防ぐ。小修正や直列依存のみの仕事は分割しない。この分担は Cloud 起動、新しいユーザー所有タスク作成、仕様変更、Human Gate の追加承認を代行しない。
