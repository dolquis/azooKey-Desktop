...
Issue には「次の AI 役割」を示す `agent:*` を 1 つ付ける。
...
- **変更カテゴリは `kind:*` を正典とする**（`feature` / `improvement` / `bug` / `docs`）。旧 `Feature` / `Improvement` / `Bug` / `enhancement` / `documentation` は廃止。`Migrated` は由来フラグとして存続。Phase 4 で旧ラベルを物理退役（ラベル削除は設定画面で手動）。
...
Tracking Issue または Project description 上部に **Next AI Task を最大 3 件**書く。
...
- **Missing Metadata**: repo / area / agent ラベルまたは GitHub リンク欠落
