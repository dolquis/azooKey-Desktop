...
| `host_health_state_tests` | `inference-host/tests/health_state_machine_test.cpp` | M47 の健康状態機械の全遷移と表外の組の拒否・自己遷移なし、model 復旧中は transport の回復で `Healthy` に戻らないこと、`SafeMode` が解除イベント以外で抜けないこと、連続クラッシュ履歴の 60 秒窓・正常終了でのリセット・時計の巻き戻り、履歴ファイルの往復と不正内容の破棄、`enteredAt` の RFC 3339 形式 |
