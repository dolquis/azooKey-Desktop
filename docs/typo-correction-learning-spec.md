...
- 既定の保存先: `%LOCALAPPDATA%\azooKey\data\typo_corrections.tsv.enc`
  （`learning.tsv.enc` / `user_dict.json.enc` と同じ `data\` 配下。
  旧平文 `typo_corrections.tsv` は移行元。host CLI `--typo-store` で論理パスを上書き可）。
- API:
