...
1. **TIP（検出・送信ゲート、一次）**: `ObserveTypo` の発火条件（§4-1 / §4-2）
   ...
   許可されるため、`QueryCandidates` 経路は学習許可では止めず
...
4. **host（適用・蓄積ゲート、二次・fail-closed）**: host は窓を見られないため
   TIP を信頼するが、防御的二重化として `secure` フラグ（§12.13 の bool）が secure を示す
   ...
   - **`learning_allowed` は `secure` と別軸**。M46 `private` /
     `custom`（`learning` OFF）モードは非 secure でも学習だけを止める
     （`privacy-and-secure-input-spec.md` §3 / §5.2 学習許可）。
   ...
   - これを正常入力のブロックなく成立させるため、privacy 対応 TIP は handshake
     ...
     必ず載せる**（§12.13）。広告済みでも欠落・型不正なら拒否する。
     学習許可には、当該接続で受理した `secure_flag` が必要である。
...
> 扱う。privacy は後方互換より優先する。

- `secure`（bool）は TIP 側 M46 secure 判定
  ...
  `secure` は protocol v1 の privacy フィールドとして定義する。
  このフィールドの追加だけで M55 の補正候補生成・適用を導入するものではない。
...
- `learning_allowed` は `secure` と別軸（§12.12.2-4）。M46 `private` /
  `custom`（`learning` OFF）は非 secure でも学習だけを止めるため、
  `secure` だけでは private 入力の学習を防げない。`QueryCandidates` も要求時点の
  `learning_allowed` を運ぶが、現在の Host はこれを M55 の補正適用ゲートでは
  消費しない。予測の `Lookup` と学習の許可は区別する。
