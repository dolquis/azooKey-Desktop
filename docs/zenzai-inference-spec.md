...
- **CPU 最適化の停止条件**: llama.cpp CPU 経路の追加最適化（DEV-857 / DEV-858 /
  DEV-859 の系統）は、**本表のいずれかの帯が「予算内」以外である間に限り継続対象**
  とする。帯の評価は次の 3 値で判定する。
  - **予算内**: その帯だけを切り出した計測が正典化され、予算を満たしている
  - **予算超過**: 同上の計測が正典化され、予算を満たしていない
  - **未評価**: その帯だけを切り出した計測が正典化されていない。読み長を混ぜた
    集合の値では帯を評価できないため、**未評価は「予算内」と扱わない**
  したがって、帯別の計測を正典化する作業（読み長別の入力別計測）自体も継続対象に
  含む。全帯が「予算内」になった後の追加短縮は、本予算を根拠とせず独立に優先度を
  判断する。`copilot-pc-backend-spec` §4.2 の 30ms 相対ゲートは、R2 artifact が
  存在しない間は継続判断の根拠にしない。
