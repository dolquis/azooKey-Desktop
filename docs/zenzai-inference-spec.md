...
- 上記 2 つの override をどう立てるかの判断は、llama.cpp の型に依存しない純ロジック
  ...
  配列末尾には llama.cpp が終端とみなす空キーのエントリを必ず残す。この分離により、
  override の適用条件と llama.cpp へ渡る値を llama.cpp 無効ビルドの unit で検証できる。
...
| unit | KV override 組み立て（§9.2）: GGUF の tokenizer メタデータから llama.cpp へ渡す override 一覧を決める純ロジックを、llama.cpp 無効ビルドで検証する。`gpt2-small-japanese-char` では `tokenizer.ggml.pre` の文字列 override が 1 件だけ立ち、上流 pre-tokenizer・pre-tokenizer 宣言なしでは立たない。pre-tokenizer と eos の両方が該当する GGUF では pre-tokenizer → eos の順に 2 件並ぶ（llama.cpp へ渡す順序を固定する） |
