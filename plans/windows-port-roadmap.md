...
> **着手条件（2026-09 確定）**: ARM64 Windows 実機の入手を本マイルストーンの着手条件に加える。
> クロスビルドジョブ（DEV-908）だけを先行させても、生成した ARM64 バイナリを検証する実機ゲート
> （DEV-909）が着手できず、検証されない成果物が増えるだけになる。再開は DEV-908 → DEV-909 の順
> とする。`windows-11-arm` ランナーでの ctest 実行（下記受け入れ条件）は従量課金を伴うため、
> 再開時に別途判断する。
...
- **受け入れ条件**:
  ...
  - 起動時 + 24h 周期で新版検出、ユーザー承認で MSI 適用（`msiexec /i /qn /norestart`、
    要再起動時は通知。spec §6.3）
  - **DEV-202 の auto-DL が承認済み（暫定確定＝上流 HF から、または著者確認後の
    GitHub 再ホスト）かつ `expected.json` ピン投入済みのとき**: 既定
  ...
  - **DEV-202 が暫定確定どまりで auto-DL 未承認、または ピン未投入のとき**: (b) DL を
    有効化せず、**(c2) ユーザー明示選択（`model.selectedPath` を `settings.json` /
    設定 UI で設定）で Zenzai がロードできること**を受け入れ条件とする（暫定確定でも
    配信元は上流 HF に固定済み・§1.6.1 だが、auto-DL の有効化は別途承認を要する）。
    ピン未投入では
    (c1) autoselect が無効なため、ファイルを置くだけ（空 `selectedPath`）では
    不合格（§1.6.1 (c2)）。DL 受け入れ項目は auto-DL 承認 + ピン投入後に適用。
...
- **変更対象**: `core/src/EtwLogger.cpp`、`core/src/CrashReporting.cpp`、
  `core/src/CrashRetention.cpp`、`settings/mvp-settings.schema.json`
...
- **受け入れ条件**:
  ...
  - `privacy.crashReportConsent = local` への明示 opt-in 後、Host クラッシュで
    `%LOCALAPPDATA%\azooKey\crashes\*.dmp` が残る
  - ダンプは許可したメタデータだけを含み、本文を含み得るメモリ・スタック・レジスタを
    収集しない（spec §8.1）
  - 既定の `privacy.crashReportConsent = off` で azooKey 管理のダンプ
...
- **実装範囲**: `docs/auto-word-registration-spec.md` の M53 追補章。
  ...
  - 物理層（spec §15。DEV-412 と同時）: double-array trie と
    2 方向の前方一致検索（common-prefix / predictive）の契約、`.azdic` v1 直列化形式、
    マルチソースビルド
...
- **参照仕様**: `docs/auto-word-registration-spec.md` M53 追補（§14 論理層。
  ...
  正規化の適用点 §15.3 / 検索 API 契約 §15.4 / `.azdic` 形式 §15.5 /
