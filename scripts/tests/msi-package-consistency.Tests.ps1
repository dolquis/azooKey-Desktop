Describe "WiX MSI package consistency" {
  ...
  It "always installs the self-contained settings payload and shortcut" {
    ...
    $script:package | Should -Match '<\?ifdef SettingsObjDirExists \?>\s*<Exclude Files="\$\(SettingsPayloadDir\)\\obj\\\*\*" />\s*<\?endif \?>'
    $script:project | Should -Match 'Condition="Exists\(''\$\(SettingsPayloadDir\)\\obj''\)"'
    $script:project | Should -Match '<DefineConstants>\$\(DefineConstants\);SettingsObjDirExists=1</DefineConstants>'
